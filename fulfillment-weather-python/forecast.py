...
class ForecastError(Exception):
    """Raised when a forecast cannot be retrieved or parsed."""
    pass
...
class Forecast(object):
    ...
    def __call_wwo_api(self, date):
        ...
        try:
            ...
            log.exception('WWO API request failed for city=%s date=%s', self.city, date)
            raise ForecastError('Unable to reach the weather service at this time.') from e

        error = weather_data.get('error')
        if error:
            raise ForecastError(error[0]['msg'])
        return weather_data
...
def parse_datetime_input(datetime_input):
    """Parse ISO-8601 date/time and intervals.

    Returns (datetime_start, datetime_end). Each value may be None if parsing fails
    or the input does not contain a date/time.
    """
    ...
    try:
        ...
        # 13:30:00/14:30:00
            ...
            datetime_input_start, datetime_input_end = datetime_input.split('/')
        ...
        # 2014-01-01/2014-12-31
            ...
            datetime_start = dt.strptime(datetime_input_start, '%Y-%m-%d').date()
        ...
        # 2017-02-08T08:00:00Z/2017-02-08T12:00:00Z
            ...
            datetime_start = dt.strptime(datetime_input_start, '%Y-%m-%dT%H:%M:%SZ')
            datetime_end = dt.strptime(datetime_input_end, '%Y-%m-%dT%H:%M:%SZ')
        else:
            datetime_start = None
            datetime_end = None
    except (TypeError, ValueError) as e:
        log.warning('Unable to parse datetime string %r: %s', datetime_input, e)
        datetime_start = None
        datetime_end = None
